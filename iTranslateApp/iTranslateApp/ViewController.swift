...
class ViewController: UIViewController {
    ...
    @IBAction func nameButton(_ sender: Any) {
        myImage.image = UIImage(named: "bonjour")
        let name = userName.text!
        nameLabel.text = " Welcome to iTranslate, \(name)! Slide the switch next to each phrase to get started."
    ...
    @IBAction func switchSix(_ sender: Any) {
        ...
        let name = userName.text!
        if (sender as AnyObject).isOn { answerSix.text = "Mon nom est \(name)"
        }
        else {
            answerSix.text = " "
        }
